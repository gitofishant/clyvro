...
function injectHtmlComponentTagger(options = {}) {
    ...
    htmlFiles.forEach((htmlFile) => {
        ...
        const injectorScriptPath = path__namespace
            .relative(path__namespace.dirname(filePath), outputFile)
            .replace(/\\/g, '/');
        const inspectorScriptName = options.inspectorScriptName || 'dhws-web-inspector.js';
        const inspectorFile = path__namespace.join(outputDir, inspectorScriptName);
        const inspectorScriptPath = path__namespace
            .relative(path__namespace.dirname(filePath), inspectorFile)
            .replace(/\\/g, '/');
        const hasInjector = content.includes('dhws-dataInjector');
        const hasInspector = content.includes('dhws-elementInspector');
        const injectorScriptTag = `<script id="dhws-dataInjector" src="${injectorScriptPath}"></script>`;
        const inspectorScriptTag = `<script id="dhws-elementInspector" src="${inspectorScriptPath}"></script>`;
        if (!hasInjector || !hasInspector) {
            let scriptsToAdd = '';
            if (!hasInjector) {
                scriptsToAdd += `${injectorScriptTag}\n`;
            }
            if (!hasInspector) {
                scriptsToAdd += `${inspectorScriptTag}\n`;
            }
            if (scriptsToAdd) {
                stats.injectedFiles++;
